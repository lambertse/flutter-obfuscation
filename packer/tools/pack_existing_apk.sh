...
if [[ "$MECHANISM_TEST" == "1" ]]; then
  ...
  echo "           guard_hook_got_symbol: hooked 'dlopen' in 'libflutter.so'"
  echo "           guard_trampoline_apply_embedded_key: embedded key installed"
  ...
  echo "     (2) 'guard_ctor: FATAL, could not hook dlopen ... aborting' + SIGABRT =>"
  echo "         libguard loaded but couldn't find libflutter mid-co-load. This is an"
  echo "         approach-internal timing issue to fix in guard_ctor, NOT a RASP/pivot"
