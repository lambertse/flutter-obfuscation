...
def render_regions_h(per_abi_regions: dict) -> str:
    lines = [
    ...
    for i, abi in enumerate(abis):
        ...
        for symbol, size, nonce, exec_flag in per_abi_regions[abi]:
            lines.append(
    ...
    lines.append("#endif")
    lines.append("")
    lines.append("static const size_t k_regions_count = sizeof(k_regions) / sizeof(k_regions[0]);")
    lines.append("")
...
def main():
    ...
    if args.key_hex:
        ...
        key = derive_key_from_cert(args.cert_der.read_bytes())
    ...
    args.output_regions_h.write_text(render_regions_h(per_abi_regions))
